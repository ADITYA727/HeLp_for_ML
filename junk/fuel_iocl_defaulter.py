...
data=pd.read_csv('/home/stealth/Desktop/analysis/whole_data_iocl.csv',sep=',')
#data=pd.read_csv('/home/stealth/Desktop/analysis/iocl_new.csv', sep=',')

"""
['S.No,', ' Terminal ID', ' Merchant Name ', ' Location',
       ' Customer ID/Card PAN', ' Vehicle No.', ' Txn Date', ' Txn Type',
       ' Product', ' Currency ', ' RSP', ' Quantity', ' Amount', ' Balance',
       'Entitled CashBack', ' Odometer', ' Status', 'Odometer_New',
       'Corrected', 'Month', 'Pan', 'Field_Status']
...
def limit_exceed_and_reaidng(data_dict):
    try:
        ...
        print(data_dict['Product_Category'])
...
def cal_mileage(hpcl, acc):
    ...
    hpcl = hpcl.sort_values(' Txn Date')
    vehicles = hpcl[col[5]].unique()
    vehicles_mileage = {}
    vehicle_df = hpcl[hpcl[col[4]] == acc]
    ...
    fuel_used = sum(map(float, vehicle_df[' Quantity'][:-1]))
    amount_spent = sum(map(float, vehicle_df[' Amount'][:-1]))
    try:
...
def make_data(df_dict,mer_account_data,acc,mer_percentage):
    ...
    for month in range(1,5):
        ...
        if mer_acc_data.shape[0]!=0:
            ...
            df_dict['Actual_Amount'] = mer_acc_data[col[12]].sum()

            df_dict['Actual_Volume'] = mer_acc_data[col[11]].sum()
...
def merchant_account_analysis(mer):
    ...
    for acc in account:
        ...
        if len(vendor)>=2:
            ...
            df_dict['Mulitple_Merchant']='Y'

            df_list2.extend(make_data(df_dict,mer_account_data,acc,mer_percentage))
        else:
            df_dict['Merchant_Name']=mer
            df_dict['Mulitple_Merchant'] = 'N'
            df_list.extend(make_data(df_dict,mer_account_data,acc,mer_percentage))
...
new_data.to_csv('/home/stealth/Desktop/analysis/iocl_mileage.csv')
